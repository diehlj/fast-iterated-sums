...
def linear_wh_channelwise(x, f, maxplus_linear=False):  # XXX I think it should be _hw
    ...
    if len(f.shape) == 1:
        if maxplus_linear:
            f_expanded = (
                f.unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
            )  # [1, channels, 1, 1]
            ...
            f_x = torch.einsum(
                "...cwh, c->...cwh", x, f
            )  # XXX I think it should be _hw
    else:
        raise ValueError(
            "Channel-wise linear does not support f with shape (out_channels, in_channels)"
        )
    return f_x


def linear_wh(
    x, f, maxplus_linear=False, is_complex=False
):  # XXX I think it should be _hw
    if is_complex:
        assert not maxplus_linear, "Maxplus linear is not supported for complex tensors"
    if len(f.shape) == 1:
        if maxplus_linear:
            f_expanded = (
                f.unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
        ...
        if maxplus_linear:
            f_expanded = (
                f.unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
            )  # [1, out_channels, in_channels, 1, 1]
...
def sum_tree(
    ...
    for direction, subtree in tree[1]:
        ...
        )  # FIXED a bug here: semiring was NOT passed
...
def test_linear_wh():
    ...
    for i in range(2):
        for j in range(2):
            for c in range(3):
                expected[0, i, j] = torch.maximum(
                    expected[0, i, j], X[0, c, i, j] + alpha[c]
                )
    assert torch.allclose(Y, expected)

    X = torch.randint(0, 103, (1, 3, 2, 2)).float()
    alpha = torch.tensor([10.0, -21.0, 3.0]).unsqueeze(0)
    Y = linear_wh(X, alpha, maxplus_linear=True)
    expected = -torch.ones(1, 2, 2) * torch.inf
    for i in range(2):
        for j in range(2):
            for c in range(3):
                expected[0, i, j] = torch.maximum(
                    expected[0, i, j], X[0, c, i, j] + alpha[0, c]
                )
    assert torch.allclose(Y, expected)
    ...
    alpha = torch.tensor([1.0, -20.0, 3.0])
    ALPHA = torch.diag(alpha)
