...
from kernels import scans_discounted_cummax, scans_discounted_cumsum
from kernels.scans_discounted_cumsum import DiscountedCumsumN_BCHW_Complex
from shared.sums import Semiring, TreeDirection, linear_wh, linear_wh_channelwise
...
def ds_cummax_S(X, discount):
    return torch.flip(
        ds_cummax_N(torch.flip(X, dims=[-2]), torch.flip(discount, dims=[-2])),
        dims=[-2],
    )
...
def ds_cummax_W(X, discount):
    return torch.transpose(
        ds_cummax_N(torch.transpose(X, -2, -1), torch.transpose(discount, -2, -1)),
        -2,
        -1,
    )
...
def ds_cummax_E(X, discount):
    return torch.flip(
        ds_cummax_W(torch.flip(X, dims=[-1]), torch.flip(discount, dims=[-1])),
        dims=[-1],
    )
...
def ds_cummax_SW(X, discount):
    return torch.flip(
        ds_cummax_NW(torch.flip(X, dims=[-2]), torch.flip(discount, dims=[-2])),
        dims=[-2],
    )
...
def ds_cummax_NE(X, discount):
    return torch.flip(
        ds_cummax_NW(torch.flip(X, dims=[-1]), torch.flip(discount, dims=[-1])),
        dims=[-1],
    )
...
def ds_cummax_SE(X, discount):
    return torch.flip(
        ds_cummax_NE(torch.flip(X, dims=[-2, -1]), torch.flip(discount, dims=[-2, -1])),
        dims=[-2, -1],
    )
...
def ds_cumsum_S(X, discount):
    return safe_flip(
        ds_cumsum_N(safe_flip(X, dims=[-2]), safe_flip(discount, dims=[-2])), dims=[-2]
    )


def ds_cumsum_W(X, discount):
    return safe_transpose(
        ds_cumsum_N(safe_transpose(X, -2, -1), safe_transpose(discount, -2, -1)), -2, -1
    )


def ds_cumsum_E(X, discount):
    return safe_flip(
        ds_cumsum_W(safe_flip(X, dims=[-1]), safe_flip(discount, dims=[-1])), dims=[-1]
    )
...
def ds_cumsum_SW(X, discount):
    return safe_flip(
        ds_cumsum_NW(safe_flip(X, dims=[-2]), safe_flip(discount, dims=[-2])), dims=[-2]
    )


def ds_cumsum_NE(X, discount):
    return safe_flip(
        ds_cumsum_NW(safe_flip(X, dims=[-1]), safe_flip(discount, dims=[-1])), dims=[-1]
    )


def ds_cumsum_SE(X, discount):
    return safe_flip(
        ds_cumsum_NE(safe_flip(X, dims=[-2, -1]), safe_flip(discount, dims=[-2, -1])),
        dims=[-2, -1],
    )


def ds_cumsum_XX(
    x: torch.Tensor, XX: str, discount: torch.Tensor, semiring: str = Semiring.REAL
) -> torch.Tensor:
...
def sum_tree(
    ...
    if callable(f):
        ...
        if channel_wise:
            f_x = linear_wh_channelwise(
                x, f, maxplus_linear=maxplus_linear, is_complex=x_is_complex
            )
        else:
            f_x = linear_wh(
                x, f, maxplus_linear=maxplus_linear, is_complex=x_is_complex
...
def sum_trees(
    ...
    for tree in trees:
        tmp = sum_tree(
            x, tree, discount, semiring=semiring, maxplus_linear=maxplus_linear
        )
        xs.append(tmp)
    if cat:
...
def test_ds_cummax_E():
    ...
    expected_Y_N = torch.tensor([[[[1.0, 0.0, 3.0], [4.0, -2.0, 1.0]]]], device="cuda")
    expected_Y_S = torch.tensor([[[[2.0, 0.0, 3.0], [4.0, -5.0, 0.0]]]], device="cuda")
    expected_Y_W = torch.tensor([[[[1.0, 0.0, 3.0], [4.0, 2.0, 0.0]]]], device="cuda")
    expected_Y_E = torch.tensor([[[[1.0, 1.0, 3.0], [4.0, -2.0, 0.0]]]], device="cuda")
...
def test_ds_cumsum_E():
    ...
    expected_Y_N = torch.tensor([[[[1.0, 0.0, 3.0], [4.5, -5.0, 1.5]]]], device="cuda")
    expected_Y_S = torch.tensor([[[[3.0, -2.5, 3.0], [4.0, -5.0, 0.0]]]], device="cuda")
    expected_Y_W = torch.tensor(
        [[[[1.0, 0.5, 3.25], [4.0, -3.0, -1.5]]]], device="cuda"
    )
    expected_Y_E = torch.tensor([[[[1.75, 1.5, 3.0], [1.5, -5.0, 0.0]]]], device="cuda")
...
def test_discount_identity():
    ...
    discounted_result_expanded = sum_tree(
        X, tree, discount_no_spatial, semiring=Semiring.REAL
    )
    undiscounted_result_no_spatial = undiscounted_sum_tree(
        X, tree, semiring=Semiring.REAL
    )
    assert not torch.allclose(
        discounted_result_expanded, undiscounted_result_no_spatial
    )  # I think it reads discount from undefined memory regions

    # expan discount_no_spatial to have spatial dimensions
    expanded_discount_no_spatial = discount_no_spatial.expand(1, 2, 3, 3)
    discounted_result_expanded = sum_tree(
        X, tree, expanded_discount_no_spatial, semiring=Semiring.REAL
    )
    undiscounted_result_no_spatial = undiscounted_sum_tree(
        X, tree, semiring=Semiring.REAL
    )
