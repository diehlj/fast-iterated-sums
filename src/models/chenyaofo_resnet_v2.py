...
'''
Uses FIS only in the downsample layer.
'''
import sys
import torch
...
class DownsampleV1(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride):
        ...
        self.alpha_1 = nn.Parameter(torch.randn(out_channels))
        self.alpha_2 = nn.Parameter(torch.randn(out_channels))
        self.tree_linear = [self.alpha_1,  [(TreeDirection.NE, [self.alpha_2, []])]]
         # nn.BatchNorm2d(planes * block.expansion)
        self.bn = nn.BatchNorm2d(out_channels)

    def forward(self, x):
        x = self.conv(x)
        # x_sum = sum_tree(x, self.tree_linear, channel_wise=True, maxplus_linear=True)
        x_sum = sum_tree(x, self.tree_linear, channel_wise=True, semiring=Semiring.MAXPLUS)
        x = self.bn(x + x_sum)
        return x

    def __str__(self):
        return "DownsampleV1(tree=" + pretty_str_tree_one_line(self.tree_linear) + ")"
...
class CifarResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = DownsampleV1(self.inplanes, planes * block.expansion, stride)
