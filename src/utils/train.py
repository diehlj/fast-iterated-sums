...
def _convert_none_strings(obj):
    """Recursively convert string 'None' to Python None in config objects."""
    if isinstance(obj, DictConfig):
        # Convert DictConfig to dict, process, then convert back
        result = {}
        for k, v in obj.items():
            result[k] = _convert_none_strings(v)
        return DictConfig(result)
    elif isinstance(obj, (list, ListConfig)):
        return type(obj)([_convert_none_strings(item) for item in obj])
    elif isinstance(obj, str) and obj == 'None':
        return None
    else:
        return obj
...
def process_config(config: DictConfig) -> DictConfig: # TODO because of filter_keys, this is no longer in place
    ...
    # XXX HACK. Convert string 'None' to actual None (fixes wandb sweep issue)
    config = _convert_none_strings(config)
...
def print_config(
    ...
    for field in fields:
        ...
        branch.add(rich.syntax.Syntax(branch_content, "yaml", background_color='white'))

    # console = rich.console.Console()
    rich.print(tree)  # or "black on white"
    # console.print(tree, style="info")  # or "black on white"
